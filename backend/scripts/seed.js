...
// ==========================================
// RINGO seed data — minimal set to test Business Trip workflow end-to-end
// Run AFTER migrate: `npm run migrate && npm run seed`
// ==========================================
...
const BUSINESS_TRIP_SETTLEMENT_SCHEMA = {
  fields: [
    { name: 'actual_amount', label: '実際合計金額 (円)', type: 'number', required: true },
    { name: 'transportation_fee', label: '交通費 (円)', type: 'number', required: false },
    { name: 'accommodation_fee', label: '宿泊費 (円)', type: 'number', required: false },
    { name: 'food_fee', label: '食事代 (円)', type: 'number', required: false },
    { name: 'other_fee', label: 'その他 (円)', type: 'number', required: false },
...
async function seed() {
  ...
  try {
    ...
    const deptRows = await client.query(`
      ...
      RETURNING id, code
    `);
    const deptByCode = Object.fromEntries(deptRows.rows.map((r) => [r.code, r.id]));

    // ---- users ----
    // Admin uses real email so they can login immediately after seed.
    // Default dev password: Ringo2026! (change via /admin after first login)
    ...
    const usersData = [
      ...
      // ── Your admin account ── login: h-bekal@jmc-ltd.co.jp / Ringo2026!
    ...
    for (const u of usersData) {
      ...
      userIdByEmail[u.email] = r.rows[0].id;
    }

    // ---- form_templates ----
    const patternId3 = (await client.query(`SELECT id FROM workflow_patterns WHERE code = 'PATTERN_3'`)).rows[0].id;
    ...
    // ---- template_permissions: BUSINESS_TRIP available to DX, SOUMU, KENPO, BIYOU ----
    ...
    // ---- approval_routes: predetermined route for BUSINESS_TRIP from DX dept ----
    // RINGI stage: User → Approver1 (Manager) → Approver2 (GM) → done (auto-issue app number)
    ...
    const ringiRouteId = ringiRoute.rows[0].id;
    ...
    // SETTLEMENT stage: User → A1 → A2(dept) → 総務 → 専務/社長(確認) → 経理処理
    ...
    const settlementRouteId = settlementRoute.rows[0].id;
